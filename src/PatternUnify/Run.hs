...
solveEntries conf !es  =
  let --intercalate "\n" $ map show es
    ...
    (result, ctx) = trace ("Initial context:\n" ++ initialContextString ++ "\n" ++ List.intercalate "\n" (map (show . infoString) es) ) $
       (runContextual ( Context B0  (map Right es) (error "initial problem ID") Empty.empty [] Set.empty conf Map.empty Map.empty) $ do
          initialise
          ambulando [] [] Map.empty
    ...
    Context lcx rcx lastProb _ finalBadEdges _ _ _ _ = unsafePerformIO $ do
...
getContextErrors startEntries cx = do
  let leftErrors = getErrorPairs (trail $ contextL cx)
      rightErrors = getErrorPairs (Either.rights $ contextR cx)
