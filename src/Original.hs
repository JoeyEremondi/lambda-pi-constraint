    ...
    iType_' ii g (Pi_ tyt tyt')
       =  do  cType_ ii g tyt VStar_
              ...
              cType_  (ii + 1) ((\ (d,g) -> (d,  ((Local ii, ty) : g))) g)
                        (cSubst_ 0 (builtin $ Free_ (Local ii)) tyt') VStar_
              return VStar_
    iType_' ii g (Free_ x)
      =     case lookup x (snd g) of
              Just ty        ->  return ty
              Nothing        ->  throwError ("unknown identifier: " ++ render (iPrint_ 0 0 (builtin $ Free_ x)))
    iType_' ii g (e1 :$: e2)
...
cType_ ii g (L region ct) = cType_' ii g ct
  where
    ...
    cType_' ii g (Lam_ e) ( VPi_ ty ty')
      =     cType_  (ii + 1) ((\ (d,g) -> (d,  ((Local ii, ty ) : g))) g)
                    (cSubst_ 0 (builtin $ Free_ (Local ii)) e) ( ty' (vfree_ (Local ii)))
